...
const char * gHashFunction =
	"static inline tHash %sHashChar( tHash hash, const char c )\n"
	"{\n"
	"    return ( hash ^ ((hash * %d) + %sMap[ (unsigned char)c ].byte) );\n"
	"}\n";
...
void printMap( void )
{
	fprintf( gOutputFile, "unsigned char %sMap[] = {\n", gPrefix );
	for ( int i = 0; i < 256; i++ )
	{
		char slug[32];
				...
				if ( config_setting_type( element ) == CONFIG_TYPE_STRING )
				{
					...
					if ( keyword != NULL )
					{
						...
						*dest = '\0';
						...
						fprintf( gOutputFile, "    %s%-16s = 0x%016lx%c    /* %s */\n",
